...
def eng_uzb(request):
    ...
    amount = Memorization.objects.filter(profil__user=request.user)

    question_lar = eval(amount[0].question_lar)
    length = len(question_lar)
    couple = length // 6
    while 1:
        q_s = Word.objects.filter(book=amount[0].book).order_by('?').first().id
        ...
        if q_s not in eval(amount[0].question_lar):
    ...
    all_respons = [soz.name for soz in
                         Soz.objects.filter(word__book=amount[0].book).order_by('?')[:3]]
